...
void MSA::init(Mat l, Mat r) {
	...
	/*the imgL/imgR have been tested*/
	...
	/*只有mst的时候用的图像是滤波后的，计算不同视差的cost时的图像是原始图像*/
	/*感觉对两边建图应该是在ctmf后啊！！！*/
...
void MSA::getCost() {
	...
	/*当j-d<0时说明有occlusion这时应该弄成和R(i,0)相减而不是和0相减*/
	for (int i = 0; i < n; ++i) {
		int occ = T(i, 0);
		for (int j = 0; j < m; ++j) {
			for (int d = 0; d < Disp; ++d) {
				...
				//costL[T(i,j)*Disp+d] = weight_col * dif_col + (1 - weight_col) * dif_gra;
				pL[T(i, j)*Disp + d] = weight_col * dif_col + (1 - weight_col) * dif_gra;
			}
		}
	}
	/*tested*/
	/*当右边occlusion的时候，等于右图同一个点前一个视差的cost*/
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m; ++j) {
			for (int d = 0; d < Disp; ++d) {
				//if (j+d<m) costR[T(i,j)*Disp+d] = costL[T(i,j+d)*Disp+d];
				//else costR[T(i,j)*Disp+d] = costR[T(i,j)*Disp+d-1];
...
/*这里的img3[]得是ctmf后的*/
void MSA::build(int rt, const double r_gra[], const double c_gra[], const uchar img3[]) {
	heap.tot = 0;
	totE = 0;
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m - 1; ++j) {
			...
			//}
			/*
			int dif_gra = r_gra[t1] - r_gra[t2];
			//if ((dif_col<=3&&abs(dif_gra) <= 3)||(abs(dif_gra)<=1 ) ) {
			if (abs(dif_gra) <= 1 && dif_col <= 1){
			//if (dif_col <= 1) {
			//if ((dif_col<=8&&abs(dif_gra) <= 3)||(dif_col>8&&abs(dif_gra)<=1 ) ) {//针对cones
			Insert(t1,t2,dif_col);
			Insert(t2,t1,dif_col);
			}
			else if (dif_gra < 0) Insert(t2,t1,dif_col);
	...
	for (int i = 0; i < n; ++i) {
	for (int j = 0; j < m; ++j) {
	pict.at<Vec3b>(i,j) = Vec3b(0,0,0);
	}
	}
	memset(pic_used,0,sizeof(bool)*n*m);
	...
	for (int j = 0; j < m; ++j) {
	int t = Find(T(i,j));
	pict.at<Vec3b>(i,j) = Vec3b(col[t][0],col[t][1],col[t][2]);
	}
	}
	imshow("SegByCol",pict);
	test*/
}
...
void MSA::Tarjan(int nn) {
	...
	int k = root[i];
	int y = k/m, x = k%m;
	circle(seg,Point(x,y),2,Scalar(0,0,255),1);
	}
	imshow("SegByGra",seg);
	*/
	/*test
...
void MSA::baseSort1(const uchar img3[]) {
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m - 1; ++j) {
			...
			//if (dif_col > 2) all_dis += min(dif_col*2,20);
			//if (dif_size > 30) all_dis += min(dif_size,50);
			/*加上两种边并且第二种dif_col/5后Tsukuba和Teddy的效果比前三种算法好，Cones略差一点，但Venus差很多*/
			/*加上getRoot后，Cones基本伯仲之间，但Venus仍然差很多*/
			/*能想到的改进，要么在连接区域的时候下功夫，要么想想一致性检验后处理occ上如何改进*/
...
void MSA::Kruskal1(const uchar img3[]) {
	...
	for (int i = 0; i < totE && totB < (n*m - 1) * 2; ++i) {
		...
		int area = 50;
		/*lim=area=50是当前最好的结果*/
		/*因为c基本是0，所以加1是弱化效果，而且效果大大增加*/
		/*现在问题是这个参数对第一个数据表现很好，对其他数据表现很差*/
		//if (fu != fv && (abs(s_u - s_v) <= lim) && (s_u <= area || s_v <= area)) {
			if (fu != fv &&c<min(tu, tv) && (abs(s_u - s_v) <= lim) && (s_u <= area || s_v <= area)) {
			//if (fu != fv&& (abs(s_u - s_v) <= lim) && (s_u <= area || s_v <= area)) {
			//if (fu != fv) {
			/*m_edg[]下标为root中的下标，表示该区域中权最大的边*/
...
void MSA::getSize(const uchar img3[]) {
	...
	for (int k = 0; k < n*m; ++k) {
		int u = seq[k];
		int rt = root[anc[u]];//seq0求出来后的anc[]为在root[]中的序号，不是实际点的标号
		++size[rt];
		for (int i = 0; i < 3; ++i) {
...
void MSA::getSeq() {
	...
	*/
	queue[0] = root[0];
	fa[root[0]] = -1;//原来fa[0] = -1;这样是不是有问题啊
	head = 0;
	tail = -1;
...
void MSA::LRcheck(uchar d1[], const uchar d2[], Mat cost) {
	...
	//if (mask[u] || mask[v]) {
	//branch[i].c += abs(d1[u]-d1[v])*5;
	//}
	//else if (!mask[u] && !mask[v]) {
	//branch[i].c += 0;
	//}
	//}
	/*motify-end*/
...
void MSA::output(uchar d0[], int scale, bool Save) {
	Mat ans;
...
void MSA::solve(Mat l, Mat r, int d, int scale, bool Save) {
	...
	//build(n*m,graR,m_img3R);
	build(n*m, r_graR, c_graR, m_img3R);
	clock_t start, finish, mid;
	start = clock();
	Tarjan(n*m);
	mid = clock();
	double dura1 = (double)(mid - start) / CLOCKS_PER_SEC;
	start = clock();
	getSeq0();
	//getRoot();
	//Prim(m_img3R);
